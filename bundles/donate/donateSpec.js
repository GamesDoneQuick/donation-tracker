...
describe('#Donate', () => {
  ...
  beforeEach(() => {
    ...
    ReactDOM.render(
      <React.Fragment>
        ...
        <table data-form="bidsform" data-form-type="empty">
          <tbody>
            <tr>
              <td>
                <input
              ...
              <td>
                <input
                  id="id_bidsform-__prefix__-customoptionname"
                  maxLength="64"
                  name="bidsform-__prefix__-customoptionname"
                  type="text"
                />
              </td>
              <td>
  ...
  describe('error handling', () => {
    it('does not allow submit if amount is below minimum', () => {
      subject = render();
      TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('input[name=amount]'), {
        target: { value: 4 },
      });
    ...
    it('does not allow submit if a suggestion is too long', () => {
      ...
      TestUtils.Simulate.change(node.querySelectorAll('[data-aid=incentives] input[type=checkbox]')[0], {
        target: { checked: true },
      });
      TestUtils.Simulate.change(node.querySelector('[data-aid=incentives] input[name=newOptionValue]'), {
        target: { value: 'Turquoise' },
      });
    ...
    it('does not allow submit if not all money is allocated', () => {
      ...
      TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('input[name=new_amount]'), {
        target: { value: 4 },
      });
    ...
    it('does not allow submit if there are too many incentives present', () => {
      ...
      [...Array(11).keys()].forEach(i => {
        TestUtils.Simulate.click(
          ReactDOM.findDOMNode(subject).querySelectorAll('[data-aid=incentives] [data-aid=result]')[i],
        );
        TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('input[name=new_amount]'), {
          target: { value: 1 },
        });
    ...
    it('does not allow submit if an expired incentive is present', () => {
      subject = render({
        ...
        formErrors: {
          bidsform: [{ bid: 'Bid does not exist or is closed.' }],
          commentform: {},
        },
        initialForm: {
          amount: '5.00',
  ...
  describe('happy paths', () => {
    ...
    it('all fields filled in', () => {
      ...
      TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('input[name=requestedalias]'), {
        target: { value: 'skavenger' },
      });
      TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('input[name=requestedemail]'), {
        target: { value: 'nobody@example.com' },
      });
      TestUtils.Simulate.change(ReactDOM.findDOMNode(subject).querySelector('textarea[name=comment]'), {
        target: { value: 'Greetings from Germany!' },
      });
      TestUtils.Simulate.submit(ReactDOM.findDOMNode(subject));
      expect([...formData.entries()].sort((a, b) => a[0].localeCompare(b[0]))).toEqual([
        ['amount', '5'],
    ...
    it('incentives', () => {
      ...
      TestUtils.Simulate.click(node.querySelector('#add'));
      TestUtils.Simulate.click(node.querySelectorAll('[data-aid=incentives] [data-aid=result]')[1]);
      TestUtils.Simulate.change(node.querySelectorAll('[data-aid=incentives] input[type=checkbox]')[1], {
        target: { checked: true },
      });
      TestUtils.Simulate.change(node.querySelector('input[name=new_amount]'), { target: { value: 5 } });
      TestUtils.Simulate.click(node.querySelector('#add'));
  ...
  function expectSubmitDisabled(message) {
    expect(ReactDOM.findDOMNode(subject).querySelector('#skip_incentives').disabled).toBe(
      true,
      'skip was not disabled',
    );
