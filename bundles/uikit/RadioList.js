...
const RadioList = props => {
  const { options, value, look = RadioListLooks.INLINE, onChange, renderOption = null, className } = props;
  ...
  return (
    <div className={classNames(styles.container, look, className)}>
      {look === RadioListLooks.CUSTOM
        ...
        : options.map(option => (
            <RadioItem {...option} key={option.value} selected={value === option.value} onSelect={handleClick} />
          ))}
    </div>
  );
};
