...
class OrderTarget extends React.Component {
  render() {
    const { target, targetProps, connectDragSource, nullOrder, spinning } = this.props;
      ...
      <Spinner spinning={spinning}>
        {connectDragSource(
          <span style={{ cursor: 'move' }}>
            {target ? (
              [
                <TargetType key="before" before={true} {...targetProps} />,
                <TargetType key="after" before={false} {...targetProps} />,
                nullOrder ? (
                  <img key="null" src={STATIC_URL + 'admin/img/icon_deletelink.gif'} onClick={nullOrder} />
                ) : null,
              ]
            ) : (
