...
export async function addDonationToGroup(donationId: number, group: string) {
  const response = await HTTPUtils.patch<APIDonation>(Endpoints.DONATIONS_GROUPS(donationId, group));
  return response.data;
}

export async function removeDonationFromGroup(donationId: number, group: string) {
  const response = await HTTPUtils.del<APIDonation>(Endpoints.DONATIONS_GROUPS(donationId, group));
  return response.data;
}
