...
export async function getUnprocessedDonations(eventId: number, options: DonationsFilterOptions = {}) {
  const response = await HTTPUtils.get<PaginationInfo<Donation>>(Endpoints.DONATIONS_UNPROCESSED(eventId), {
...
export async function getFlaggedDonations(eventId: number, options: DonationsFilterOptions = {}) {
  const response = await HTTPUtils.get<PaginationInfo<Donation>>(Endpoints.DONATIONS_FLAGGED(eventId), {
    after: options.after?.toISOString(),
  });
  return response.data.results;
}
...
export async function getUnreadDonations(eventId: number, options: DonationsFilterOptions = {}) {
  const response = await HTTPUtils.get<PaginationInfo<Donation>>(Endpoints.DONATIONS_UNREAD(eventId), {
    after: options.after?.toISOString(),
  });
  return response.data.results;
}
...
export async function getDonations(donationIds: number[]) {
  const response = await HTTPUtils.get<PaginationInfo<Donation>>(Endpoints.DONATIONS, {
    id: donationIds,
  });
  return response.data.results;
}
