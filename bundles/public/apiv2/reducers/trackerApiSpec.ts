  ...
  describe('sockets', () => {
    ...
    describe('bids', () => {
      ...
      it('processes chains', async () => {
        ...
        expect(newFlatChain.total).toBe(flatChain.total + flatChain.goal! + 50);
        expect(newFlatSteps[0].total).toBe(flatSteps[0].total + 50);
        expect(newTreeChain.total).toBe(treeChain.total + treeChain.goal! + 50);
        expect(newTreeChain.chain_steps?.[0].total).toBe((treeChain.chain_steps?.[0].total ?? 0) + 50);
      });
      ...
      it('processes children and updates parent totals', async () => {
        ...
        expect(newFlatParent?.count).toBe(flatParent.count + 1);
        expect(newFlatParent?.total).toBe(flatParent.total + 25);
        const newFlatAccepted = newFlatData.find(b => b.id === flatAccepted.id);
        expect(newFlatAccepted?.count).toBe(flatAccepted.count + 1);
        expect(newFlatAccepted?.total).toBe(flatAccepted.total + 25);
