...
import { apiRootSlice, TrackerAllQueryKeys, trackerApi, TrackerQueryKeys } from '@public/apiv2/reducers/trackerApi';
...
describe('trackerApi', () => {
  ...
  beforeEach(() => {
    ...
    mock.onGet('//testserver/' + Endpoints.DONATIONS({ eventId: 1 })).reply(() => [
      ...
      getFixturePagedDonations([
        ...
        { id: 2, comment: 'This is another comment.', pinned: true, commentstate: 'FLAGGED', groups: ['barfoo'] },
      ]),
    ]);
  });
  ...
  describe('mutations', () => {
    ...
    describe('donations', () => {
      ...
      describe('simple mutations', () => {
        const actionMap: [
          keyof typeof trackerApi.endpoints,
        ...
        actionMap.forEach(([mutation, fields, endpoint]) => {
          it(mutation, async () => {
            const donation = getData('donations', params).find(d => !shallowEqual(d, { ...d, ...fields }));
            expect(donation).toBeDefined();
            const otherDonation = getData('donations', params).find(d => d.id !== donation?.id);
      ...
      it('remove group', async () => {
        ...
        store.dispatch(
          trackerApi.endpoints.removeDonationFromGroup.initiate({ donationId: otherDonation.id, group: 'barfoo' }),
        );
        expect(getData('donations', params)[1].groups).not.toContain('barfoo');
        // wait for failure
        let data = await nextData('donations', params);
        expect(data[1].groups).toContain('barfoo');
        expect(data[0]).toBe(donation);

        // doesn't actually need any data, just a success response
        mock
          .onDelete('//testserver/' + Endpoints.DONATIONS_GROUPS({ donationId: otherDonation.id, group: 'barfoo' }))
          .reply(() => [200, null]);
        ...
        // wait for success
        data = await nextData('donations', params);
        expect(data[1].groups).not.toContain('barfoo');
        expect(data[0]).toBe(donation);
      });
    });
  });
  ...
  function getData<
    const K extends TrackerAllQueryKeys,
  ...
  async function nextData<
    const K extends TrackerQueryKeys,
    const Params extends Parameters<(typeof trackerApi.endpoints)[K]['select']>[0],
  >(
    k: K,
    params: Params | void,
