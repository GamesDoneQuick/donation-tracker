...
const Prizes = () => {
  ...
  const [getPrizes, { data: prizes, error: prizesError, isFetching: prizesFetching }] = useLazyPrizesQuery();
  // the list doesn't need it but the cards do
  const [getRuns, { error: runsError, isFetching: runsFetching }] = useLazyRunsQuery();
  const soonInterval = React.useMemo(() => Interval.after(now, Duration.fromObject({ hours: 4 })), [now]);
  const closingPrizes = React.useMemo(
    () =>
      (prizes || [])
        .filter((prize): prize is TimedPrize => {
          return prize.end_draw_time != null && soonInterval.contains(prize.end_draw_time);
        })
