...
const PrizeDetail = (props: PrizeProps) => {
  ...
  const { data: event, id: eventId, error: eventError, isFetching: eventFetching } = useEventFromRoute();
  const [getRuns, { data: runs, error: runsError, isFetching: runsFetching }] = useLazyRunsQuery();
  const [orderedRuns] = useSplitRuns(runs);

  const [getPrizes, { data: prize, error: prizeError, isFetching: prizeFetching }] = useLazyPrizesQuery({
  ...
  const handleDonate = React.useCallback(() => {
    ...
    RouterUtils.navigateTo(navigate, TrackerRoutes.EVENT_DONATE(prize.event), {
  ...
  const handleBack = React.useCallback(() => {
    if (prize) {
      RouterUtils.navigateTo(navigate, TrackerRoutes.EVENT_PRIZES(prize.event));
    }
  }, [navigate, prize]);
  ...
  return (
    <Container size={Container.Sizes.WIDE}>
      {eventFetching || runsFetching || prizeFetching ? (
