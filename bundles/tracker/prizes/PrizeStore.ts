...
export const getPrizeIds = createSelector(
  [getPrizesState],
  state => _.map(state.prizes, (_prize, id) => parseInt(id)),
);

export const getPrizes = createSelector(
  [getPrizesState],
  state => Object.values(state.prizes),
);
...
export const getSortedPrizes = createSelector(
  [getPrizes],
  prizes => prizes.sort((prize1, prize2) => TimeUtils.compare(prize1.startDrawTime, prize2.startDrawTime)),
);

export const getPrizesOpeningSoon = createSelector(
  [getPrizes, getTargetTime],
  (prizes, targetTime) => {
...
export const getPrizesClosingSoon = createSelector(
  [getPrizes, getTargetTime],
  (prizes, targetTime) => {
    const soonInterval = Interval.after(targetTime, SOON_DURATION);
    return prizes
