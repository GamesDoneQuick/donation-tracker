...
function runFromNestedAPIRun(prefix: string, fields: { [field: string]: any }): Run | undefined {
  ...
  return {
    ...
    startTime: TimeUtils.parseTimestamp(runFields.starttime),
    endTime: TimeUtils.parseTimestamp(runFields.endtime),
...
function prizeFromAPIPrize({ pk, fields }: { pk: number; fields: { [field: string]: any } }): Prize {
  ...
  return {
    ...
    endTime: TimeUtils.parseTimestamp(fields.endtime),
    startDrawTime: TimeUtils.parseTimestamp(fields.start_draw_time),
    endDrawTime: TimeUtils.parseTimestamp(fields.end_draw_time),
