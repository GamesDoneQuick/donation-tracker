...
def _dump_changed(changed):
    for c in sorted(changed, key=lambda c: c.order or math.inf):
        print(
            c.id,
            c.order,
            c.name,
            c.starttime.isoformat() if c.order else None,
            c.endtime.isoformat() if c.order else None,
            c.anchor_time is not None,
        )
