...
from .filters import PrizeEmailStateFilter, PrizeListFilter
...
class PrizeAdmin(EventArchivedMixin, RelatedUserMixin, CustomModelAdmin):
    ...
    list_filter = ('event', 'category', 'state', PrizeListFilter, PrizeEmailStateFilter)
    fieldsets = [
        (
            None,
            {
    ...
    def get_queryset(self, request):
        return (
            ...
            .prefetch_related('tags', 'claims__winner')
            .select_related(
                'category',
                'event',
                'startrun',
    ...
    def claimed(self, obj):
        if obj.key_code:
            return len([k for k in obj.prize_keys.all() if k.prize_claim_id])
        else:
            return 'N/A'

    def unclaimed(self, obj):
        if obj.key_code:
            return len([k for k in obj.prize_keys.all() if k.prize_claim_id is None])
        else:
            return 'N/A'

    def winners_(self, obj):
        return len(obj.get_winners())

    def bidrange(self, obj):
        parts = [obj.minimumbid]
        if obj.maximumbid is not None and obj.minimumbid != obj.maximumbid:
            parts.append(obj.maximumbid)
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj and obj.key_code:
            readonly_fields += ('maxwinners', 'maxmultiwin', 'requiresshipping')

        if not (
            request.user.has_perm('tracker.can_search_for_user')
            or request.user.has_perm('tracker.view_user')
        ):
    ...
    @method_decorator(permission_required('tracker.change_prize'))
    ...
    @method_decorator(
        permission_required(
            ('tracker.change_prizewinner', 'tracker.view_donor', 'tracker.change_prize')
        )
    )
    ...
    @method_decorator(
        permission_required(('tracker.change_prizewinner', 'tracker.change_prize'))
    )
