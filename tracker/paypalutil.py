    ...
    with contextlib.suppress(ValueError):
        ...
        if donation:
            ...
            try:
                ...
                return None
            # if the donation amount does not match, the form payload is corrupted somehow and cannot be trusted
            if donation.amount == ipn.mc_gross:
                return donation
            viewutil.tracker_log(
                'paypal', f'IPN ignored, amount did not match {ipn.id}'
            )
    return None
...
def initialize_paypal_donation(sender, **kwargs):
    ...
    if donation.event.locked:
        viewutil.tracker_log(
            'paypal', f'Not processing IPN due to event being locked. #`{ipn.id}`.'
        )
        return
