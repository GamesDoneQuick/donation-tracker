...
class Command(commandutil.TrackerCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-e', '--event', help='Specify the event',
        ...
        parser.add_argument('-t', '--template',
                            help='specify an email template', default=None)
    ...
    def handle(self, *args, **options):
        ...
        if emailTemplate == None:
            self.message(
                "No default prize winner accepted email template specified on event {0}, cannot send e-mails.".format(event.short))
        else:
