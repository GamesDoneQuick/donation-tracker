...
class Command(commandutil.TrackerCommand):
    ...
    def add_arguments(self, parser):
        ...
        group.add_argument(
            '-l',
            '--list',
            help='List all prize winners which have passed expiry',
            action='store_true',
        )
        group.add_argument(
            '-c',
            '--close',
            help='Close all prize winners which have passed expiry (does not re-roll)',
            action='store_true',
        )
        parser.add_argument(
            '-d',
            '--dry-run',
            help='Run the command, but do not commit anything to the database',
            action='store_true',
        )
