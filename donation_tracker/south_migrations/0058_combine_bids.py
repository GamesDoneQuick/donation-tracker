...
class Migration(DataMigration):
  ...
  def forwards(self, orm):
    for dbid in orm['donation_tracker.DonationBid'].objects.all():
      others = orm['donation_tracker.DonationBid'].objects.filter(bid=dbid.bid, donation=dbid.donation)
      if others.count() > 1:
        print 'Combining %d bids on %s' % (others.count(), dbid.bid.name)
        for o in others:
          print o.id,o.amount
        newbid = orm['donation_tracker.DonationBid'].objects.create(bid=dbid.bid, donation=dbid.donation,
                                                           amount=sum([o.amount for o in others], 0))
        print newbid.id,newbid.amount
        others.delete()
