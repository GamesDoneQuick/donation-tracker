...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'DonorCache'
        db.create_table(u'tracker_donorcache', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('event', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['donation_tracker.Event'], null=True, blank=True)),
            ('donor', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['donation_tracker.Donor'])),
            ('donation_total', self.gf('django.db.models.fields.DecimalField')(default=0, max_digits=20, decimal_places=2)),
            ('donation_count', self.gf('django.db.models.fields.IntegerField')(default=0)),
            ('donation_avg', self.gf('django.db.models.fields.DecimalField')(default=0, max_digits=20, decimal_places=2)),
            ('donation_max', self.gf('django.db.models.fields.DecimalField')(default=0, max_digits=20, decimal_places=2)),
        ))
        db.send_create_signal('donation_tracker', ['DonorCache'])

        # Adding unique constraint on 'DonorCache', fields ['event', 'donor']
        db.create_unique(u'tracker_donorcache', ['event_id', 'donor_id'])

        if not db.dry_run:
            donations = orm['donation_tracker.Donation'].objects.filter(transactionstate='COMPLETED').exclude(donor=None)
            donors = orm['donation_tracker.Donor'].objects.all()
            events = orm['donation_tracker.Event'].objects.all()
            i,j = 0,donors.count()
            for d in donors:
                i += 1
                print ('%8d\%8d\r' % (i,j)),
                sys.stdout.flush()
                for e in events:
                  aggregate = donations.filter(event=e,donor=d).aggregate(total=Sum('amount'),count=Count('amount'),max=Max('amount'),avg=Avg('amount'))
                  if aggregate['count']:
                    cache,c = orm['donation_tracker.DonorCache'].objects.get_or_create(event=e,donor=d)
                    self.update_cache(cache, aggregate)
                aggregate = donations.filter(donor=d).aggregate(total=Sum('amount'),count=Count('amount'),max=Max('amount'),avg=Avg('amount'))
                if aggregate['count']:
                  cache,c = orm['donation_tracker.DonorCache'].objects.get_or_create(event=None,donor=d)
                  self.update_cache(cache, aggregate)
	
    def update_cache(self, cache, aggregate):
        cache.donation_total = aggregate['total']
        cache.donation_count = aggregate['count']
        cache.donation_max = aggregate['max']
        cache.donation_avg = aggregate['avg']
        cache.clean()
        cache.save()

    def backwards(self, orm):
        # Removing unique constraint on 'DonorCache', fields ['event', 'donor']
        db.delete_unique(u'tracker_donorcache', ['event_id', 'donor_id'])

        # Deleting model 'DonorCache'
        db.delete_table(u'tracker_donorcache')
    ...
    models = {
        ...
        'donation_tracker.donorcache': {
            'Meta': {'unique_together': "(('event', 'donor'),)", 'object_name': 'DonorCache'},
