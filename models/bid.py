...
class BidManager(models.Manager):
    def get_by_natural_key(self, event, name, speedrun=None, parent=None):
        return self.get(event=Event.objects.get_by_natural_key(*event),
                        name=name,
                        speedrun=SpeedRun.objects.get_by_natural_key(
                            *speedrun) if speedrun else None,
...
class Bid(mptt.models.MPTTModel):
    ...
    def clean(self):
        ...
        if self.id is None or (sameName.exists() and sameName[0].state == 'HIDDEN' and self.state == 'OPENED'):
