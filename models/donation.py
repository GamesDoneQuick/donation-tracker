...
LanguageChoices = (('un', 'Unknown'), ('en', 'English'),
                   ('fr', 'French'), ('de', 'German'))
...
class Donation(models.Model):
    ...
    donor = models.ForeignKey(
        'Donor', on_delete=models.PROTECT, blank=True, null=True)
    ...
    fee = models.DecimalField(decimal_places=2, max_digits=20, default=Decimal(
        '0.00'), validators=[positive], verbose_name='Donation Fee')
    currency = models.CharField(
        max_length=8, null=False, blank=False, choices=_currencyChoices, verbose_name='Currency')
    ...
    requestedvisibility = models.CharField(max_length=32, null=False, blank=False, default='CURR', choices=(
        ('CURR', 'Use Existing (Anonymous if not set)'),) + DonorVisibilityChoices, verbose_name='Requested Visibility')
    ...
    requestedsolicitemail = models.CharField(max_length=32, null=False, blank=False, default='CURR', choices=(
        ('CURR', 'Use Existing (Opt Out if not set)'), ('OPTOUT', 'Opt Out'), ('OPTIN', 'Opt In')), verbose_name='Requested Charity Email Opt In')
    commentlanguage = models.CharField(max_length=32, null=False, blank=False,
                                       default='un', choices=LanguageChoices, verbose_name='Comment Language')
...
class Donor(models.Model):
    ...
    firstname = models.CharField(
        max_length=64, blank=True, verbose_name='First Name')
    lastname = models.CharField(
        max_length=64, blank=True, verbose_name='Last Name')
    visibility = models.CharField(
        max_length=32, null=False, blank=False, default='FIRST', choices=DonorVisibilityChoices)
    ...
    solicitemail = models.CharField(max_length=32, choices=(
        ('CURR', 'Use Existing (Opt Out if not set)'), ('OPTOUT', 'Opt Out'), ('OPTIN', 'Opt In')), default='CURR')
...
class DonorCache(models.Model):
    ...
    donation_total = models.DecimalField(decimal_places=2, max_digits=20, validators=[
                                         positive, nonzero], editable=False, default=0)
    donation_count = models.IntegerField(
        validators=[positive, nonzero], editable=False, default=0)
    donation_avg = models.DecimalField(decimal_places=2, max_digits=20, validators=[
                                       positive, nonzero], editable=False, default=0)
    donation_max = models.DecimalField(decimal_places=2, max_digits=20, validators=[
                                       positive, nonzero], editable=False, default=0)
    ...
    def donation_update(sender, instance, **args):
        ...
        if cache.donation_count:
            ...
            cache.delete()
        cache, c = DonorCache.objects.get_or_create(
            event=None, donor=instance.donor)
        cache.update()
        if cache.donation_count:
            cache.save()
        else:
    ...
    def update(self):
        ...
        aggregate = aggregate.aggregate(total=Sum('amount'), count=Count(
