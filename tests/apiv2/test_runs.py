...
class TestRunMove(TestSpeedRunBase, APITestCase):
        ...
        for n, (a, b) in enumerate(compat.pairwise(ordered), start=1):
            with self.subTest(f'ordered {n}: {a}, {b}'):
                ...
                self.assertEqual(
                    a.endtime,
                    b.starttime,
                    msg=f'Run times for {a.id} and {b.id} do not match',
                )
    ...
    def test_across_anchor_before(self):
        ...
        self.assertRunsInOrder(
            [self.run3, self.run1, self.run2, self.run5, run6, run7, run8], [self.run4]
        )
    ...
    def test_across_anchor_after(self):
        ...
        # run2 should adjust setup time
        # 1, 5, and 6 should adjust starttime
        # run1, 2, and 3 should change order

        self.assertResults(self.run1, after=self.run3, expected_change_count=5)
        self.assertRunsInOrder(
            [self.run2, self.run3, self.run1, self.run5, run6, run7, run8], [self.run4]
        )
    ...
    def test_suborder_collision(self):
        ...
        self.assertResults(
            ...
            before=self.run1,
            expected_status_code=400,
            expected_error_codes={'suborder': 'invalid'},
        )
