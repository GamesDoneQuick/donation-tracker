...
class TestTrackerSettings(TestCase):
    ...
    def test_checks(self):
        ...
        self.assert_check('tracker.E114', TRACKER_REGISTRATION_FROM_EMAIL=1)
        self.assert_check(
            'tracker.E115', TRACKER_REGISTRATION_FROM_EMAIL='not.an.email'
        )
        self.assert_check('tracker.E116', TRACKER_VOLUNTEER_REGISTRATION_FROM_EMAIL=1)
        self.assert_check(
            'tracker.E117', TRACKER_VOLUNTEER_REGISTRATION_FROM_EMAIL='not.an.email'
        )
        self.assert_check('tracker.E118', TRACKER_PUBLIC_SITE_ID='foo')
        self.assert_check('tracker.E118', TRACKER_PUBLIC_SITE_ID=500)
        site = Site.objects.create(domain='a.site', name='A Site')
        self.assert_check('-tracker.E118', TRACKER_PUBLIC_SITE_ID=site.id)
        # the other Sites checks are extreme edge cases so we will not test those here
        self.assert_check('tracker.W119', Warning, DOMAIN='example.com')
        self.assert_check(
            'tracker.W120', Warning, TRACKER_DEFAULT_PRIZE_COORDINATOR='unknown'
        )
        self.assert_check(
            '-tracker.W120', Warning, TRACKER_DEFAULT_PRIZE_COORDINATOR='coordinator'
        )
        self.assert_check('tracker.E120', TRACKER_DEFAULT_PRIZE_COORDINATOR=1)
