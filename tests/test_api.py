    ...
    def assertModelPresent(self, expected_model, data):
        ...
        for model in data:
            if model['pk'] == expected_model['pk'] and model['model'] == expected_model['model']:
                ...
                break
        if not found_model:
            raise AssertionError('Could not find model "%s:%s" in data' % (
                expected_model['model'], expected_model['pk']))
        extra_keys = set(found_model['fields'].keys()) - \
            set(expected_model['fields'].keys())
    ...
    def assertModelNotPresent(self, unexpected_model, data):
        ...
        for model in data:
            if model['pk'] == unexpected_model['pk'] and model['model'] == unexpected_model['model']:
                ...
                break
        if not found_model:
            raise AssertionError('Found model "%s:%s" in data' % (
...
class TestGeneric(APITestCase):
    ...
    def test_add_log(self):
        request = self.factory.post(
            '/api/v1/add', dict(type='runner', name='trihex', stream='https://twitch.tv/trihex'))
        request.user = self.super_user
        ...
        self.assertEqual(add_entry.content_type,
    ...
    def test_change_log(self):
        old_runner = models.Runner.objects.create(
            name='PJ', youtube='TheSuperSNES')
        request = self.factory.post('/api/v1/edit', dict(type='runner', id=old_runner.id,
                                                         name='trihex', stream='https://twitch.tv/trihex', youtube=''))
        request.user = self.super_user
    ...
    def test_change_log_m2m(self):
        run = models.SpeedRun.objects.create(
            name='Test Run', run_time='0:15:00')
        runner1 = models.Runner.objects.create(name='PJ')
        runner2 = models.Runner.objects.create(name='trihex')
        request = self.factory.post(
            '/api/v1/edit', dict(type='run', id=run.id, runners='%s,%s' % (runner1.name, runner2.name)))
        request.user = self.super_user
    ...
    def test_delete_log(self):
        ...
        self.assertFalse(models.Runner.objects.filter(
            pk=old_runner.pk).exists())
        entry = LogEntry.objects.order_by('pk').last()
...
class TestSpeedRun(APITestCase):
    ...
    def format_run(cls, run):
        return dict(
            fields=dict(
                ...
                endtime=format_time(
                    run.endtime) if run.endtime else run.endtime,
                event=run.event.id,
    ...
    def test_get_single_run(self):
        request = self.factory.get(
            '/api/v1/search', dict(type='run', id=self.run1.id))
    ...
    def test_get_starttime_lte(self):
        request = self.factory.get(
            '/api/v1/search', dict(type='run', starttime_lte=format_time(self.run2.starttime)))
        request.user = self.user
        data = self.parseJSON(tracker.views.api.search(request))
    ...
    def test_get_starttime_gte(self):
        request = self.factory.get(
            '/api/v1/search', dict(type='run', starttime_gte=format_time(self.run2.starttime)))
        request.user = self.user
        data = self.parseJSON(tracker.views.api.search(request))
    ...
    def test_get_endtime_lte(self):
        request = self.factory.get(
            '/api/v1/search', dict(type='run', endtime_lte=format_time(self.run2.endtime)))
        request.user = self.user
        data = self.parseJSON(tracker.views.api.search(request))
        self.assertEqual(len(data), 2)
        self.assertModelPresent(self.format_run(self.run1), data)
    ...
    def test_get_endtime_gte(self):
        request = self.factory.get(
            '/api/v1/search', dict(type='run', endtime_gte=format_time(self.run2.endtime)))
        request.user = self.user
        data = self.parseJSON(tracker.views.api.search(request))
        self.assertEqual(len(data), 3)
        self.assertModelPresent(self.format_run(self.run2), data)
    ...
    def test_add_with_category(self):
        request = self.factory.post('/api/v1/add', dict(type='run', name='Added Run With Category',
                                                        run_time='0:15:00', setup_time='0:05:00', category='100%'))
    ...
    def test_edit_with_runners_as_names_invalid(self):
        request = self.factory.post('/api/v1/edit', dict(
            type='run', id=self.run2.id, runners='%s,%s' % (self.runner1.name, 'nonsense')))
        request.user = self.add_user
        data = self.parseJSON(tracker.views.api.edit(request), status_code=400)
        self.assertEqual(
    ...
    def test_add_with_new_category(self):
        ...
        request = self.factory.post('/api/v1/add',
                                    dict(type='prize',
                                         ...
                                         handler=json.dumps(
                                             self.add_user.natural_key()),
                                         category='Grand'))
        request.user = self.add_user
        data = self.parseJSON(tracker.views.api.add(request))
        self.assertEqual(len(data), 1)
        self.assertEqual(models.Prize.objects.get(
            pk=data[0]['pk']).category, models.PrizeCategory.objects.get(name='Grand'))
...
class TestEvent(APITestCase):
    ...
    def test_event_annotations(self):
        models.Donation.objects.create(
            event=self.event, amount=10, domainId='123456')
        models.Donation.objects.create(
        ...
        data = self.parseJSON(tracker.views.api.search(request))
        self.assertEqual(len(data), 2)
        event_data = next(e for e in data if e['pk'] == self.locked_event.id)[
            'fields']
        ...
        event_data = next(e for e in data if e['pk'] == self.event.id)[
            'fields']
