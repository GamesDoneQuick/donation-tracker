...
class TestAdminFilters(TestCase):
    ...
    def test_run_event_filter(self):
        ...
        f = RunEventListFilter(request, {'run': [runs[0].id]}, models.Bid, BidAdmin)
        self.assertQuerySetEqual(
            f.queryset(request, models.Bid.objects.filter(event=event)),
        ...
        with self.assertRaises(IncorrectLookupParameters):
            ...
            RunEventListFilter(request, {}, models.Bid, BidAdmin)
        with self.assertRaises(IncorrectLookupParameters):
            request = self.factory.get(
                '/whatever', {'event__id__exact': event.id, 'run': 'foo'}
            )
            RunEventListFilter(
                request, {'run': ['foo']}, models.Bid, BidAdmin
            ).queryset(request, models.Bid.objects.all())
