...
class TestParseValue(TransactionTestCase):
    ...
    def test_single_pk_fetch(self):
        self.assertEqual(parse_value(
            models.SpeedRun, 'event', self.event.pk), self.event)
    ...
    def test_single_natural_key_as_string_fetch(self):
        self.assertEqual(parse_value(models.SpeedRun, 'event',
                                     self.event.short), self.event)

    def test_single_natural_key_as_json_fetch(self):
        self.assertEqual(parse_value(models.SpeedRun, 'event',
                                     json.dumps(self.event.natural_key())), self.event)

    def test_single_natural_key_as_string_create(self):
        runner = parse_value(models.Submission, 'runner',
                             'UraniumAnchor', self.super_user)
    ...
    def test_complex_natural_key_as_json_create(self):
        run = parse_value(models.Bid, 'speedrun',
                          '["Mega Man 3", ["%s"]]' % self.event.short, self.super_user)
