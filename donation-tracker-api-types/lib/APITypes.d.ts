declare module 'APITypes' {
  ...
  type Slug = string; // alphanumerics, dash, underscore -- for certain fields (e.g. Tag lists), also normalize to lowercase
  type URLString = string;
  type Duration = string; // e.g. '1:15:00' for an hour and 15 minutes
  type ISOTimestamp = string;
  type FieldSet<T> = T | { [P in keyof T]: undefined };

  export type EventAPIId = SingleKey;
  export type TalentAPIId = SingleKey;
  export type RunAPIId = number | [string, string, [string]];

  export type Event = ModelBase<'event'> & {
    short: Slug;
    name: string;
    hashtag: string;
    ...
    receiver_logo: URLString;
    receiver_privacy_policy: URLString;
  ...
  export interface TreeBid extends Omit<BidBase, 'event' | 'repeat' | 'allowuseroptions'> {
    event?: number;
    allowuseroptions?: boolean;
    options?: BidChild[];
  ...
  export interface Prize extends ModelBase<'prize'> {
    ...
    image: URLString;
    altimage: URLString;
    imagefile: null | URLString;
    estimatedvalue: null | number;
    minimumbid: number;
    sumdonations: boolean;
    provider: string;
    creator: null | string;
    creatorwebsite: null | URLString;
  }
