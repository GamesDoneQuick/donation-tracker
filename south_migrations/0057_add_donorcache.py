...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('tracker_donorcache', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('event', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['tracker.Event'], null=True, blank=True)),
        ...
        db.send_create_signal('tracker', ['DonorCache'])

        # Adding unique constraint on 'DonorCache', fields ['event', 'donor']
        db.create_unique('tracker_donorcache', ['event_id', 'donor_id'])

        if not db.dry_run:
            donations = orm['tracker.Donation'].objects.filter(transactionstate='COMPLETED').exclude(donor=None)
            donors = orm['tracker.Donor'].objects.all()
            events = orm['tracker.Event'].objects.all()
            i,j = 0,donors.count()
            for d in donors:
                i += 1
                print(('%8d\%8d\r' % (i,j)), end=' ')
                sys.stdout.flush()
                for e in events:
                  aggregate = donations.filter(event=e,donor=d).aggregate(total=Sum('amount'),count=Count('amount'),max=Max('amount'),avg=Avg('amount'))
                  if aggregate['count']:
                    cache,c = orm['tracker.DonorCache'].objects.get_or_create(event=e,donor=d)
                ...
                if aggregate['count']:
                  cache,c = orm['tracker.DonorCache'].objects.get_or_create(event=None,donor=d)
                  self.update_cache(cache, aggregate)
    ...
    def backwards(self, orm):
        ...
        db.delete_unique('tracker_donorcache', ['event_id', 'donor_id'])

        # Deleting model 'DonorCache'
        db.delete_table('tracker_donorcache')
