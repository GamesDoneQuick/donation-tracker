...
class Migration(DataMigration):
  ...
  def forwards(self, orm):
    for dbid in orm['tracker.DonationBid'].objects.all():
      others = orm['tracker.DonationBid'].objects.filter(bid=dbid.bid, donation=dbid.donation)
      if others.count() > 1:
        print('Combining %d bids on %s' % (others.count(), dbid.bid.name))
        for o in others:
          print(o.id,o.amount)
        newbid = orm['tracker.DonationBid'].objects.create(bid=dbid.bid, donation=dbid.donation,
